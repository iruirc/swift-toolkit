...
| [`feature-estimation`](skills/feature-estimation/SKILL.md) | Convert work-items into a calibrated day range using feature-type defaults, selective PERT, scope-aware additive risk deltas, confidence/maturity labels, delivery-calendar conversion, and optional project overrides. Produces `## Estimation` in `Plan.md`. Always a scenario range, never a point. Optional AI-assisted mode derives a second, calibration-gated range. |
| [`mobile-ops-checklist`](skills/mobile-ops-checklist/SKILL.md) | Cross-cutting validation checklist (16 categories: release, state, networking, offline, crash, perf, push, deeplinks, a11y, i18n, analytics, privacy, migrations, testing, 3p SDK, CI/CD). Each item Applicable (with evidence) / N/A (with reason) / Pending. Produces a separate `OpsChecklist.md` artifact. |
...
When a run is at `live`, token cost is reported alongside every stage: model, generated
tokens, context size, tool calls and elapsed time, plus a totals line when the run finishes.
`normal` is unchanged from before — no per-stage cost line. The skill path carries no run id to
report against either, so `/swift-agents --all` and `scripts/agent-monitor.sh` still show it on
demand, reading the same transcripts. All three read Claude Code's own transcripts and write
nothing.
