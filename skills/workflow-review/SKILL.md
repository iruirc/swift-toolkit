...
description: "Воркфлоу профиля REVIEW: одиночный проход swift-reviewer + auto-move в DONE/ при APPROVED. Активируется swift-toolkit:orchestrator-ом, не вызывается пользователем напрямую."
---
...
- `need_test`, `need_review` — игнорируются: сам этот профиль и есть ревью; повторно `swift-reviewer` себя не вызывает, а тесты выполняются в воркфлоу-донорах (FEATURE/BUG/REFACTOR/TEST).
- `mode` — `manual` / `auto` (см. секции 3 и 4): влияет только на согласование auto-move.
- `stack`, `archive_paths` — стандартно (контекст для `swift-reviewer` и информация о бэкапах, сделанных оркестратором).

## 2. Stages

- **Review** — `swift-reviewer`. Артефакт: `Review.md` в папке задачи. **Обязательная первая строка** артефакта: `[REVIEW_STATUS] = APPROVED | CHANGES_REQUESTED | DISCUSSION` — это контракт со всеми воркфлоу и оркестратором. Тело `Review.md`: что сделано хорошо, что требует изменений (по severity), открытые вопросы. Подробное описание выходного формата — в `agents/swift-reviewer.md`.
...
- `status=error` — ошибка контракта (например, некорректный `start_stage`, отсутствует/некорректен `[REVIEW_STATUS]`, фатальный сбой `swift-reviewer` или `task-move`).
