...
- **Plan** — `swift-toolkit:swift-architect`. Artifact: `Plan.md` with **two layers of progress tracking**:
  ...
  The architect MUST apply the `feature-estimation` skill to produce an additional `## Estimation` section in `Plan.md` (feature type + baseline table + PERT rows where needed + scope-aware risk deltas + engineering range + confidence + estimate maturity + delivery-calendar conversion + assumptions + known unknowns + self-check). The estimation range is a hard-prerequisite for entering Execute. If `## Estimation` is missing/malformed, `### Estimate maturity` is `Draft`, the maturity is `Conditional` and its named conditions have not been explicitly accepted/deferred/resolved by the user, or a Known Unknown could swing the estimate >30% without a required spike/resolution, Plan stays open and the workflow returns `ask_user`.
