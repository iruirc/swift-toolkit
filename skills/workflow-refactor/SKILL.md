...
- **Refactor** — `swift-refactorer` (см. `agents/swift-refactorer.md`). Поэтапно применяет рефакторинг по фазам из `Plan.md`, обновляя прогресс-таблицу после каждой фазы. После каждой фазы (по возможности) запускает локальные тесты и фиксирует прогресс. Артефакт стадии — изменения в исходниках; отдельного `.md`-файла Refactor не создаёт. **Никаких изменений внешнего поведения** — этот инвариант проверяется в Validation.

  Если в args передан `start_phase=<phase_id>` — `swift-refactorer` получает эту фазу как точку старта в промпте Task tool. Уже завершённые фазы (статус `✅` в `Plan.md`) пропускаются, не переделываются. Прогресс-таблица обновляется только для новых/изменённых фаз.
